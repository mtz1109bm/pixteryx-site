...
export default function Quote() {
  ...
  const { daysMin, daysMax, priceMin, priceMax } = useMemo(() => {
    ...
    // Nombre de fonctionnalités (±15% entre 3 et 12)
    const fMul = 1 + ((features - 6) * 0.03); // de ~0.88 à ~1.18
        ...
        <div className="mt-8 grid md:grid-cols-2 gap-6">
          ...
          <form onSubmit={handleSubmit} className={`rounded-2xl border border-white/10 p-6 ${brand.card} grid gap-4`}>
              ...
              <div className="grid grid-cols-3 gap-2">
                {(["low","med","high"] as Complexity[]).map(c => (
                  <button type="button" key={c}
                    ...
                    className={`px-3 py-2 rounded-xl border ${complexity===c ? "border-sky-400/60 bg-sky-500/10" : "border-white/10"}`}>
              ...
              <div className="grid grid-cols-3 gap-2">
                {(["mvp","std","xl"] as Scope[]).map(s => (
                  <button type="button" key={s}
                    onClick={()=>setScope(s)}
                    className={`px-3 py-2 rounded-xl border ${scope===s ? "border-sky-400/60 bg-sky-500/10" : "border-white/10"}`}>
              ...
              <div className="grid grid-cols-3 gap-2">
                {(["flex","normal","rush"] as Deadline[]).map(d => (
                  <button type="button" key={d}
                    ...
                    className={`px-3 py-2 rounded-xl border ${deadline===d ? "border-sky-400/60 bg-sky-500/10" : "border-white/10"}`}>
            ...
            <div className="grid gap-1">
              ...
              <input type="range" min={3} max={12} value={features} onChange={e=>setFeatures(parseInt(e.target.value))}
            ...
            <div className="grid gap-1">
              ...
              <input value={name} onChange={e=>setName(e.target.value)}
                     className={`px-3 py-2 rounded-xl bg-slate-800/60 border ${errors.name?"border-red-500":"border-white/10"}`} placeholder="Votre nom" />
            ...
            <div className="grid gap-1">
              ...
              <input value={email} onChange={e=>setEmail(e.target.value)} type="email"
                     className={`px-3 py-2 rounded-xl bg-slate-800/60 border ${errors.email?"border-red-500":"border-white/10"}`} placeholder="vous@entreprise.com" />
            ...
            <div className="grid gap-1">
              ...
              <textarea value={notes} onChange={e=>setNotes(e.target.value)} rows={4}
                        className="px-3 py-2 rounded-xl bg-slate-800/60 border border-white/10" placeholder="Contexte, objectifs, contraintes…" />
            </div>
          ...
          <div className="rounded-2xl border border-white/10 p-6 bg-slate-900/40 h-fit">
            ...
            <ul className="mt-4 space-y-2 text-slate-300">
              ...
              <li>Service : {LABELS[service]} — Complexité : {complexity==="low"?"Simple":complexity==="med"?"Moyenne":"Élevée"}</li>
              <li>Portée : {scope==="mvp"?"MVP":"Standard" /* else */}{scope==="xl"?" Étendue":""} — Échéance : {deadline==="rush"?"Urgent":deadline==="flex"?"Flexible":"3–8 sem."}</li>
